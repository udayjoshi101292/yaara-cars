...
function car_list($conn, $arry_, $column, $filter, $limit = '', $single = false, $asc = 'Modal', $brand = ''){
    ...
    WHERE $column IN ($arry_list) $brand_ $modal_ AND yc_master.Location = 'KSA' AND yc_engine.Price != 'Discontinued' AND yc_modal.Status_Modal = 'Publish' AND yc_engine.Status = 'Publish' GROUP BY $filter_list ORDER BY $asc ASC $limit";
...
function car_fitler($conn, $arry_, $column, $filter, $Brand_Slug = '', $limit = '', $asc = 'Modal'){
    ...
    WHERE $column IN ($arry_list) AND yc_master.Location = 'KSA' AND yc_engine.Price != 'Discontinued' AND yc_modal.Status_Modal = 'Publish' AND yc_engine.Status = 'Publish' $Brand GROUP BY $filter_list ORDER BY $asc ASC $limit";
...
function brand_logo($conn, $arry_, $limit){
    ...
    $list_all .= "SELECT * FROM yc_master WHERE Brand IN ($arry_list) AND yc_master.Location = 'KSA' GROUP BY Brand ORDER BY FIELD(Brand, $arry_list) $limit";
...
function price_carousel($conn, $car_price, $limit = ''){
    ...
    INNER JOIN yc_modal ON yc_master.Master_ID = yc_modal.Brand_ID 
    INNER JOIN yc_engine ON yc_modal.Mod_ID = yc_engine.Modal_ID
    WHERE yc_master.Location = 'KSA' AND yc_engine.Price != 'Discontinued' AND yc_engine.Price != 'TBD' AND yc_modal.Status_Modal = 'Publish' AND yc_engine.Status = 'Publish' GROUP BY Modal ORDER BY Modal ASC";
...
INNER JOIN yc_interior ON yc_engine.Var_ID = yc_interior.Variant_ID 
INNER JOIN yc_measurement ON yc_engine.Var_ID = yc_measurement.Variant_ID 
INNER JOIN yc_safety ON yc_engine.Var_ID = yc_safety.Variant_ID WHERE yc_master.Location = 'KSA' AND yc_engine.Price != 'Discontinued' AND yc_modal.Status_Modal = 'Publish' AND yc_engine.Status = 'Publish'";
