...
function sitemap_generator($name, $urls, $frequency = 'hourly', $priority = '0.9') {
  ...
  $filepath = dirname(__FILE__,2);
    ...
    foreach ($urls as $key => $url) {
      $xmlString .=  '<url><loc>'.$url.'</loc><lastmod>'.date('c').'</lastmod><changefreq>'.$frequency.'</changefreq><priority>'.$priority.'</priority></url>';
    }
