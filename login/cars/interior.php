...
$column_name = car_data($conn, $default = array('table_name' => 'yc_interior'));
$data = get_data_data_testing($conn, 'yc_interior', 'ID', null, $page, null, null, $filter_emp_arr);

$get_filter_01 = get_data_data_testing($conn, 'yc_interior', null, null, null, ' GROUP BY Center_Arm_Rest ', ' Center_Arm_Rest ');
$get_filter_02 = get_data_data_testing($conn, 'yc_interior', null, null, null, ' GROUP BY Cargo_area_light ', ' Cargo_area_light ');

$count_data = get_data_data_testing($conn, 'yc_interior', 'ID', true, null, null, null, $filter_emp_arr) / 10;
...
<section class="py-4">
    <div class="row">
        <div class="col-md-12 ">
            <h2 class="section_titles">All Interior</h2>
        </div>
    </div>
    ...
    <form action="" method="GET">
        <div class="filter_wrap ">
            <?php echo  select_filters($get_filter_01,'Center Arm Rest', 'Center_Arm_Rest', array('name' => 'Center_Arm_Rest')) ?>
            <?php echo  select_filters($get_filter_02,'Cargo Area Light	', 'Cargo_area_light', array('name' => 'Cargo_area_light')) ?>
    ...
    <form action="../export-import.php" method="post" class="mt-2" enctype="multipart/form-data">
        ...
        <input type="hidden" value="yc_interior-ID" name="table-name">
    </form>
                                ...
                                foreach ($arr_sliced_col as $k => $v) { ?>
                                    <?php if ($v['COLUMN_NAME'] === 'Content'): ?>
                                        <td class="table_data_text post_title_td">
                                            <textarea name="brand_ID" disabled rows="2"><?php echo  $value[$v['COLUMN_NAME']]; ?> </textarea>
                                    ...
                                    <?php else: ?>
                                        <td class="table_data_text post_title_td">
                                            <input type="text" name="brand_ID" value="<?php echo  $value[$v['COLUMN_NAME']]; ?>" disabled>
                                            <!-- <div class="hidden"><?php // echo  $value[$v['COLUMN_NAME']]; 
                                                                        ?></div> -->
                                        </td>
                                    <?php endif; ?>
