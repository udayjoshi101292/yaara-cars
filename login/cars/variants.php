...
$column_name = car_data($conn, $default = array('table_name' => 'yc_engine'));
$data = get_data_data_testing($conn, 'yc_engine', 'Var_ID', null, $page, null, null, $filter_emp_arr);

$get_locations = get_data_data_testing($conn, 'yc_engine', null, null, null, ' GROUP BY Variant  ', ' Variant ');
$get_status = get_data_data_testing($conn, 'yc_engine', null, null, null, ' GROUP BY Status  ', ' Status ');

$count_data = get_data_data_testing($conn, 'yc_engine', 'Var_ID', true, null, null, null, $filter_emp_arr) / 10;
...
<section class="py-4">
    <div class="row">
        <div class="col-md-12 ">
            <h2 class="section_titles">All Variants</h2>
        </div>
    </div>
    ...
    <form action="" method="GET">
        <div class="filter_wrap ">
            <?php echo  select_filters($get_locations,'Variant', 'Variant', array('name' => 'Variant')) ?>
            <?php echo  select_filters($get_status,'Status', 'Status', array('name' => 'Status')) ?>
    ...
    <form action="../export-import.php" method="post" class="mt-2" enctype="multipart/form-data">
        ...
        <input type="hidden" value="yc_engine-Var_ID" name="table-name">
    </form>
    ...
    <form action="" method="POST">
        ...
        <div class="container-fluid section-container my-3 leads_table database_tables_wrap table-style">
                                ...
                                foreach ($arr_sliced_col as $k => $v) { ?>
                                    <?php if ($v['COLUMN_NAME'] === 'Content'): ?>
                                        <td class="table_data_text post_title_td">
                                            <textarea name="brand_Var_ID" disabled rows="2"><?php echo  $value[$v['COLUMN_NAME']]; ?> </textarea>
                                    ...
                                    <?php else: ?>
                                        <td class="table_data_text post_title_td">
                                            <input type="text" name="brand_Var_ID" value="<?php echo  $value[$v['COLUMN_NAME']]; ?>" disabled>
                                            <!-- <div class="hidden"><?php // echo  $value[$v['COLUMN_NAME']]; 
                                                                        ?></div> -->
                                        </td>
                                    <?php endif; ?>
            ...
            //     echo "Treu";
            //     print_r($_SERVER['SCRIPT_URI'].'?'.$_SERVER['QUERY_STRING'].'&page='. 1);
            // }else{
            //     echo "False";
            //     print_r($_SERVER['SCRIPT_URI'].'?'.'page='.'1');
            // }
            ?>
