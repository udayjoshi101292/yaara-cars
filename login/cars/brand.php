...
$column_name = car_data($conn, $default = array('table_name' => 'yc_master'));
$data = get_data_data_testing($conn, 'yc_master', 'Master_ID', null, $page, null, null, $filter_emp_arr);

$get_locations = get_data_data_testing($conn, 'yc_master', null, null, null, ' GROUP BY Location  ', ' Location ');
$get_brands = get_data_data_testing($conn, 'yc_master', null, null, null, ' GROUP BY Brand ', ' Brand ');
$get_year = get_data_data_testing($conn, 'yc_master', null, null, null, ' GROUP BY Year ', ' Year ');
$get_currency = get_data_data_testing($conn, 'yc_master', null, null, null, ' GROUP BY Currency', ' Currency ');

$count_data = get_data_data_testing($conn, 'yc_master', 'Master_ID', true, null, null, null, $filter_emp_arr) / 10;
...
<section class="py-4">
    <div class="row">
        <div class="col-md-12 ">
            <h2 class="section_titles">All Brands</h2>
        </div>
    </div>
    ...
    <form action="" method="GET">
        <div class="filter_wrap ">
            <?php echo  select_filters($get_locations,'Location', 'Location', array('name' => 'location')) ?>
            <?php echo  select_filters($get_brands,'Brand','Brand', array('name' => 'brand')) ?>
            <?php echo  select_filters($get_year,'Year','Year', array('name' => 'year')) ?>
            <?php echo  select_filters($get_currency,'Currency','Currency', array('name' => 'currency')) ?>
    ...
    <form action="../export-import.php" method="post" class="mt-2" enctype="multipart/form-data">
        ...
        <input type="hidden" value="yc_master-Master_ID" name="table-name">
    </form>
                                ...
                                foreach ($arr_sliced_col as $k => $v) { ?>
                                    <?php if ($v['COLUMN_NAME'] === 'Content'): ?>
                                        <td class="table_data_text post_title_td">
                                            <textarea name="brand_Master_ID" disabled rows="2"><?php echo  $value[$v['COLUMN_NAME']]; ?> </textarea>
                                    ...
                                    <?php else: ?>
                                        <td class="table_data_text post_title_td">
                                            <input type="text" name="brand_Master_ID" value="<?php echo  $value[$v['COLUMN_NAME']]; ?>" disabled>
                                            <!-- <div class="hidden"><?php // echo  $value[$v['COLUMN_NAME']]; 
                                                                        ?></div> -->
                                        </td>
                                    <?php endif; ?>
