...
$modalDetails = GetDataFor($conn, 'yc_modal', 'Brand_ID', $pageId, false, " AND Status_Modal ='Publish' ", " GROUP BY Modal_Slug ORDER BY MODAL ASC ", null, null, true);
                    ...
                    <div class="mk_select_model_holder">
                        <select class="js-example-basic-single" name="make-modal-select" id="" data-select-var="variant">
                            ...
                            <?php foreach ($modalDetails as $md) {
                                if (GetDataFor($conn, 'yc_engine', 'Ref_Modal', $md['Modal'], true, "  AND Status='Publish' AND Var_Location='$location' AND Price !='DISCONTINUED'", "", null, null, true) !== false) {
        ...
        <div class="row">
            <div class="col-md-8">
                ...
                foreach ($modalDetails as $md) {
                    // $variantsDetails = GetDataFor($conn, 'yc_engine', 'Ref_Modal', $md['Modal'], true, " AND Status='Publish' AND Var_Location='$location' AND Price !='DISCONTINUED'", "", null, null, true);
                    
                    $modal = $md['Modal'];
                    $queryCondition = " AND Ref_Modal = '$modal' AND Status='Publish' AND Var_Location='$location' AND Price != 'DISCONTINUED'";

                    $variantsDetails = GetDataFor($conn, 'yc_engine', 'Ref_Brand', $md['Brand_ID'], true, "  $queryCondition", "", null, null, true);
                
                ?>
                <!-- <div style="display: none;">
                    <pre>
                        <?php  //var_dump($variantsDetails);?>
                    </pre>
                </div> -->
                    <?php
                ...
                <div class="yc-single-car-content news-bar p-3 mb-3">
                    ...
                    <div class="yc-trend-car-desc row mt-4">
                        <div class="col-12">
                            ...
                            <?php $post_list = get_post($conn, 'knowledge-hub', 6); ?>
