...
<?php $title_slug = $car_slug; 
    ...
    if(count($car_slug) == 3){
        ...
        $car_title = car_list($conn, [end($title_slug)],'Modal_Slug',['Brand', 'Modal'],'');
    } 
    elseif(in_array('images', $title_slug) || in_array('variants', $title_slug)) {
        array_pop($title_slug);
        $car_title = car_list($conn, [end($title_slug)],'Brand_Slug',['Brand', 'Modal'],'');   
    } 
    else {
        $car_title = car_list($conn, [end($title_slug)],'Modal_Slug',['Brand', 'Modal'],'');
    }
            ...
            <div class="yc-single-tab-wrap">
                <ul class="yc-single-tab-lists">
                    <li class="yc-single-tab-item <?php echo $car_active; ?>">
                        <a href="<?php echo site_url().'/'.$car_title[0]['Brand_Slug'].'/'.$car_title[0]['Modal_Slug']; ?>"><?php echo $car_title[0]['Brand']; ?></a>
                    </li>
